...
def train_and_test(_):
    ...
    W1 = tf.Variable(tf.truncated_normal(
        shape=(NUM_PIXELS, NUM_HIDDEN1), stddev=0.1))
    b1 = tf.Variable(tf.constant(0.1, shape=(NUM_HIDDEN1,)))

    W2 = tf.Variable(tf.truncated_normal(
        shape=(NUM_HIDDEN1, NUM_HIDDEN2), stddev=0.1))
    b2 = tf.Variable(tf.constant(0.1, shape=(NUM_HIDDEN2,)))

    W3 = tf.Variable(tf.truncated_normal(
        shape=(NUM_HIDDEN2, NUM_CLASSES), stddev=0.1))
    ...
    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(labels=y, logits=h3))
