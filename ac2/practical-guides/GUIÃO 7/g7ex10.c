...
void configUart(unsigned int baud, char parity, unsigned int stopbits){
	...
	//	1. Configurar o gerador baudrate
		if(baud >= 600 && baud <= 115200){
			U1BRG = ((PBCLK + 8 * baud)/(16 * baud));		// constant calculation
		}else{
			U1BRG = ((PBCLK + 8 * 115200)/(16 * 115200));	// default baud
		}
...
char getc(void){
	...
	// com interrupções não esquecer PERR e FERR;
